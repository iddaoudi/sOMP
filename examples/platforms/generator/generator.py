...
zone = [SubElement(socket, 'zone', id='NUMA-'+str(i), routing='Cluster') for i in range(numa)]
numa_counter = 0
ncpu_counter = 0
for z in zone:
    prop1 = SubElement(z, 'prop', id='ncache', value='FILL')
    prop2 = SubElement(z, 'prop', id='cachesize', value='FILL')
    prop3 = SubElement(z, 'prop', id='alpha', value='FILL')
    prop4 = SubElement(z, 'prop', id='beta', value='FILL')
    ...
    backbone = SubElement(z, 'backbone', id='backbone-'+str(numa_counter), bandwidth='500GBps', latency='0ns')
    numa_counter += 1
    ncpu_counter += ncpu
