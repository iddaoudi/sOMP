...
numazone = [SubElement(socket, 'zone', id='NUMA-'+str(i), routing='Floyd') for i in range(numa)]
numa_counter = 0
ncpu_counter = 0
nlink_counter = 0
nlink_counter2 = 0
...
for z in numazone:
    ...
    for i in range(ncpu):
        ...
        route = SubElement(z, 'route', src='CPU-'+str(i + ncpu_counter), dst='CPU-'+str(i+1+ncpu_counter))
        ctn = SubElement(route, 'link_ctn', id='link-'+str(i + ncpu_counter), up='link-'+str(i+ncpu_counter)+'_UP', down='link-'+str(i+ncpu_counter)+'_DOWN')
        numa_counter += numa
    ncpu_counter += ncpu
